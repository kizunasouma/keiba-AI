...
_BATCH_IDS = 200000  # 1回のSQLに渡すID数の上限（学習時高速化のため拡大）
...
def build_training_dataset(db: Session) -> pd.DataFrame:
    ...
    # 追加特徴量を計算（全特徴量をバッチ処理で計算）
    print(f"  特徴量計算開始（{len(df):,}件）...")
    import time as _t
    _t0 = _t.time()
    ...
    print(f"  v7完了: {_t.time()-_t0:.0f}秒 — 全特徴量計算完了")
